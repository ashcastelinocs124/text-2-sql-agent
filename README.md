...
Benchmark and compare SQL generation capabilities of different LLM agents using standardized tasks and scoring.

### 2. Student SQL Exam Grading

AgentX can be adapted for **automated grading of student SQL exams**:

| Feature | Benefit for Grading |
|---------|---------------------|
| **Result comparison** | Check if student query produces correct output |
| **Row/column order tolerance** | Accept equivalent answers regardless of ordering |
| **Partial matching** | Give partial credit when results partially match |
| **Hallucination detection** | Catch invalid table/column references |
| **Multi-dimensional scoring** | Grade on correctness, efficiency, best practices |
...
# Setup with exam schema
executor = SQLExecutor(ExecutorConfig(dialect="sqlite"))
# ... setup exam tables ...

# Grade student submission
student_sql = "SELECT name FROM students WHERE grade > 80"
result = executor.process_query(student_sql)

# Compare with expected results
expected = [{"name": "Alice"}, {"name": "Bob"}]
comparator = DefaultResultComparator(ignore_row_order=True)
comparison = comparator.compare(result.data, expected)

# Generate score
scorer = EnhancedScorer()
score = scorer.score(comparison=comparison, ...)

print(f"Student Score: {score.overall:.0%}")  # e.g., "Student Score: 85%"
```

**Considerations for Exam Use:**
- Currently result-based (checks output correctness, not SQL structure)
- For pattern enforcement (e.g., "must use JOIN"), additional validation rules needed
- Scoring weights can be customized per question
- Supports batch grading for multiple submissions

### 3. SQL Code Review
Validate SQL queries in CI/CD pipelines for best practices, potential errors, and performance issues.

### 4. Training Data Generation
Generate scored SQL examples for fine-tuning LLMs on SQL generation tasks.
